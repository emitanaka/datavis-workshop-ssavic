---
title: "Exercise 1 Solution: Refreshing your `ggplot2` skills"
output:
  html_document:
    toc: true
    toc_float: true
...
```{r day2-exercise-03, echo = T, class = "target", fig.height = 4}
tb_oz %>% 
...
```{r day2-exercise-04, echo = T, class = "target"}
tb_oz %>% 
  filter(year == 2012) %>%
...
```{r day2-exercise-05, echo = T, class = "target"}
tb_oz %>% 
  filter(year == 2012) %>%
...
```{r day2-exercise-06, echo = T, class = "target"}
tb_oz %>% 
  filter(year == 2012) %>%
...
```{r day2-exercise-07, echo = T, class = "target"}
tb_oz %>% 
  filter(year == 2012) %>%
...
```{r day2-exercise-08, echo = T, class = "target"}
tb_oz %>%
  group_by(year) %>%
...
```{r day2-exercise-09, echo = T, class = "target"}
...
```{r day2-exercise-10, echo = T, class = "target"}
tb_oz %>% group_by(year, age_group) %>% 
  summarise(p = count[sex=="m"]/sum(count)) %>%
  ggplot(aes(x=year, y=p)) +
    geom_hline(yintercept = 0.50, colour="white", size=2) +
